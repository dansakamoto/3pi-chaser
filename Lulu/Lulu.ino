 ...
 */
// Motor constants
const int stopSpeed = 0; // Motor speed when stopped. Probably zero
const int walkSpeed = 30; // Motor speed when walking
const int runSpeed = 60; // Motor speed when running

// sensor constants
const int threshold = 300; // Threshold of threat detection (ambient light sensor) // 650 threshold with room lights on // ??? with lights off
//const int threshold = 400; // Threshold of threat detection (proximity sensor)

// Behavioral constants
//const int maxFear = 200; // This is how scared it gets when there's a threat in front of it
const int maxFear = 100; // This is how scared it gets when there's a threat in front of it
int turn = 0;
...
void setup() {
  ...
  //pinMode(A5, INPUT); // (formerly right sensor)
  pinMode(A7, INPUT); // center sensor (formerly left sensor)
  currentIdx = 0;
...
void loop() {
  ...
  //sensorValue = 0; // FOR DEBUGGING - STOP READING VALUES FROM LIGHT SENSOR

  OrangutanLCD::clear();
  OrangutanLCD::print(turn);
  //OrangutanLCD::print(" ");
  //OrangutanLCD::print(rightValue);

  if(sensorValue > threshold){  // Sensors detects light - go backwards
    fear = maxFear;
    dir = -1; // -1 = backwards
    rightSpeed = dir * runSpeed;
    leftSpeed = dir * runSpeed;

  } else if(sensorValue <= threshold){ // no imminent threat

    if(fear > 0){ // but there was a recent threat

      // keep running
      leftSpeed = dir * runSpeed;
      rightSpeed = dir * runSpeed;
      //movSpeed = dir * runSpeed;
      turn ++;
      fear--; // but become less afraid
      
    } else { // not afraid anymore
      ...
      //movSpeed = stopSpeed;
      dir = 1; // resume forward movement

      /*
       * And resume line following
       */
      ...
      if (power_difference < 0){
        ...
        leftSpeed = maximum + power_difference;
        rightSpeed = maximum;
      }else{
        //OrangutanMotors::setSpeeds(maximum, maximum - power_difference);
        leftSpeed = maximum;
        rightSpeed = maximum - power_difference;
        }
    }
    }
